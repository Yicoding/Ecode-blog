...
router.get('/api/shop/findall', (req, res) => {
	var sql = 'select s.id, s.menu_id, s.total, s.isCheck, m.name, m.price, m.type_id, m.descript, m.picture from shop s inner join menu m on s.menu_id=m.id where s.user_id=?'
